...
class TextProcessor: 
    ...
    def __init__(self, cfg=None):
        c = cfg or config.get("preprocessing", {}).get("text_processor", {})
        self.skip_phrases = c.get("skip_substrings", [])
        self.rm_phrases = sorted(c.get("rm_substrings", []), key=len, reverse=True)
        self.bad_words = c.get("obscene_substrings", [])

    def clean(self, text):
        if not text or self.skip(text):
            return ""
        text = self.preserve_numeric_ranges(text)
        text = self.apply_rm_phrases(text)
        cleaners = [
            self.rm_emoji,
            self.rm_links,
            self.norm_quotes,
    ...
    def preserve_numeric_ranges(self, text):
    ...
    def apply_rm_phrases(self, text):
        for phrase in self.rm_phrases:
            text = text.replace(phrase, " ")
        return text
    
    def norm_punct(self, text):
